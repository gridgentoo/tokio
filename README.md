Легковесный **service mesh** написанный на Rust, **Conduit data plane** использует нижележащие Open Source-технологии: 
Tokio (событийная платформа ввода/вывода для создания сетевых асинхронных приложений) и 
Tower (лаконично описывается как fn(Request) → Future<Response>). Такой подход позволил авторам добиться минимальных задержек («sub-millisecond p99») и потребления памяти («10mb RSS»).
